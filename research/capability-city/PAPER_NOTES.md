...
| N-12 | **The invariant is not the defect.** `verifyRuntimeSeparation` behaved exactly as specified and correctly detected the historical failure in a control case. Per the round's rule, it was **not** weakened, excepted or special-cased to let the acceptance proceed. | `MEASURED` |

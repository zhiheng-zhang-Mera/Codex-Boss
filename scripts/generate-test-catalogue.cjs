...
const CURATED = {
  ...
  // The ledger's own provenance (docs/city/BOSS_CITY_CONTINUOUS_CONSTRUCTION_SUCCESSOR_WORKBOOK-2.md section 1). Cloud verification found an entry claiming a
  // `timestamp_utc` hours AFTER the commit that already contains it; this suite drives the control that finds it in
  // BOTH directions, because a control that is only ever observed passing has not been shown to control anything.
  "tests/unit/city/city-ledger-provenance.test.ts": { covers: ["runtime"], obligation: "the ledger's machine-read fields and timestamps are checkable rather than trusted: the parser reads the real entry shape and does NOT read the schema block as an entry, a strict ISO-8601 UTC value is the only thing the timestamp field may carry (prose on that line is refused), an entry's recorded instant may not be later than the committer instant of the commit that FIRST CARRIED it by more than the recorded tolerance nor more than the recorded maximum before it, timestamps may not decrease in an append-only file, and every `## CC-NNN` heading corresponds to exactly one ENTRY_ID block; disclosed history is grandfathered BY ID and not by shape, so a constructed entry carrying the CC-063 defect is still refused, every disclosed id must exist in the ledger, and the shipped CLI exits zero on the real ledger with the same entry count a fresh parse produces", alwaysRun: true },
  ...
  "tests/unit/city/principle-enforcement-validator.test.ts": { covers: ["runtime"], obligation: "the principle enforcement matrix is DATA rather than prose precisely because a matrix written as prose can claim anything, and the specific forgery section 23 invites is PROMOTION -- recording MACHINE ENFORCED beside a principle whose only guard is a regression ratchet, so that the count cannot silently grow back is written down as the count is zero; so no measurement is typed into the matrix at all: each row names a resolution KEY, and the validator resolves the live value from that guard own JSON output, which is why the drift this artifact is exposed to is impossible rather than merely untested, asserted by a case that every key the file names is a registered key, that no measured field exists anywhere in it, and that every target in it is zero; the cases build a matrix that breaks each rule and assert the rejection -- a principle with no row, a duplicated row, an invented 15.10, a strength outside the four, a row whose strength falls short of what section 23 asks without saying why, a guard that does not exist, a guard whose source writes to the tree, a guard that exits non-zero, a test that mentions none of the guards it is cited for, a guard no cited test reaches, MACHINE_ENFORCED whose measurement is above its target, MACHINE_ENFORCED with a non-zero target, a RATCHET whose measurement REACHED its target and must therefore be promoted, a key no instrument publishes, a key read from a guard the row did not name, a machine claim with nothing to measure it against, EVIDENCE_REQUIRED with no record, with an insubstantial evidence requirement and with a record that does not exist, and NOT_GUARDED that does not name the stage owning its gap; and against the real tree it asserts the honest distribution -- three enforced, two ratchets, three evidence-required and one unguarded -- that the five cited guards are read-only and exit zero when run directly, that the generated document table is identical to the table the validator generates, and that the three unguarded principles stay named rather than left to inference; the document-currency comparison is asserted to be LINE-ENDING AGNOSTIC in one case and to STILL FAIL on a one-character change in the same case, because the table is generated with LF while git checks the document out with CRLF -- which is how the FIRST CI RUN of this stage failed, on a check that passed on the machine that wrote the file and failed on the runner that verified it (ledger CC-034)", alwaysRun: true },
  // CC-043: docs/city/OWNER_CONTINUOUS_CONSTRUCTION_WORKBOOK.md section 21, the replacement lifecycle.
  "tests/unit/city/replacement-lifecycle.test.ts": { covers: ["runtime"], obligation: "section 21 requires a REUSABLE replacement-governance mechanism -- do not attempt to replace every capability merely to exercise this -- names seven pieces of machinery, and asks for one real bounded migration as its proof, and principle 15.4 was the LAST row of the enforcement matrix still NOT_GUARDED; the reason it is a state machine and not a checklist is that a replacement reporting RETIRED without ever having run the old and new sides side by side is indistinguishable IN PROSE from one that did, so the ORDER is what is checked -- every instance carries a history of the states it actually entered and the validator refuses a history that skips a state or arrives by an undeclared transition, with a case asserting the REASON in the failure: a RETIRED instance that never passed DUAL_VALIDATED is the specific forgery this refuses; the refusals are each falsified -- a skipped state, a step with no substantive evidence for the state it enters, a step citing a ledger entry the ledger lacks, a clock that runs backwards, a state CLAIMED that the history never entered, and a rollback proof file that does not exist, because rollback is executable is otherwise an assertion; the protocol is checked too, that DECLARED is first, that all six named stages exist and each requires evidence, that no transition returns to DECLARED, that terminal states have no outgoing transition and that no non-terminal state is a dead end; the rollback proof is REAL rather than a pointer: the successor declares ProviderId and the authoritative side carries EXACTLY ONE re-export pointing at it, so the old side is a PURE ALIAS and restoring it cannot change semantics; and GENERICITY IS DEMONSTRATED rather than claimed, by driving a SECOND synthetic instance for a DIFFERENT capability down the full path through the SAME validator using only the fields the protocol declares, which is section 21 acceptance that future capabilities can adopt it without inventing a new protocol", alwaysRun: true },
  // CC-042: docs/city/OWNER_CONTINUOUS_CONSTRUCTION_WORKBOOK.md section 30's acceptance suite over section 33.
  "tests/unit/city/city-final-acceptance.test.ts": { covers: ["runtime"], obligation: "section 33 of docs/city/OWNER_CONTINUOUS_CONSTRUCTION_WORKBOOK.md enumerates the conditions for completion -- nine Governance, fourteen Structure, five Evidence and six Final-main items -- and section 30 enumerates what must run and adds no final result may depend solely on a local run, so the suite carries all 34 items with unique ids and substantive text AND EVIDENCE, because a checklist entry that does not explain itself is an assertion; three statuses rather than two, and the difference is the design: PASS is machine-verified, OPEN is machine-verified as NOT satisfied and always blocks, UNVERIFIED is what the tree cannot decide and blocks UNLESS the run carries --attest AND the final acceptance record exists, since the workbook own vehicle for an Owner attestation is that record, and the cases falsify the seal decision in both directions -- ready only when nothing is OPEN and nothing is unverifiable, blocked by a single OPEN item whatever else passes, and blocked by UNVERIFIED for every combination except attest plus record, with --attest alone granting nothing; the items section 30 forbids a local run from settling are reported UNVERIFIED rather than passed, including completeness of the cloud ledger and the claim that no historical failure was erased, where the evidence says out loud that the check which can disprove it is the GIT HISTORY and not a working tree; OPEN is decided from the artifact and not from hope, proven by a root that lacks a required artifact reporting exactly those items OPEN -- the path that would have caught section 30's unwritten bridge expiry validator -- and by the road item being verified for the first time under a machine predicate while the structural items print the measurement that keeps them open; and the render never emits the final status while anything blocks", alwaysRun: true },
  // CC-041: docs/city/OWNER_CONTINUOUS_CONSTRUCTION_WORKBOOK.md section 30's missing bridge expiry validator.
  "tests/unit/city/bridge-expiry-validator.test.ts": { covers: ["runtime"], obligation: "section 30 of docs/city/OWNER_CONTINUOUS_CONSTRUCTION_WORKBOOK.md names a bridge expiry validator among the artifacts the final acceptance suite must run, section 15 requires eight fields per temporary bridge and says a bridge without an exit condition is not allowed, and section 33 puts no expired temporary bridge remains in the completion checklist -- and NONE OF IT EXISTED, so a bridge past its deadline or one whose code had already been deleted while the declaration stayed was invisible; the check that makes a declaration falsifiable is the bridge LITERAL: the validator asserts that the exact source statement still appears in its source file EXACTLY ONCE, because zero occurrences means the bridge was removed and the record is stale while more than one means the bridge is not one statement and its exit condition does not describe it; the check that makes the deadline mean anything is that a phase is RESOLVED from measurements and not from prose -- each stage carries trackedBy and decidedBy instead of a typed measurement, and a stage whose keys cannot be resolved reads UNRESOLVED and FAILS CLOSED, because an unmeasurable deadline that reads as not yet due is exactly how a bridge outlives its phase; the cases break each rule in turn, removing each of the eight fields, deleting the literal from the tree, making it appear many times, dropping it entirely, naming an unrecognised deadline, forcing a phase COMPLETE so the bridge EXPIRES, leaving a phase IN_PROGRESS and proving it was resolved from the instrument rather than typed, making a phase unmeasurable two ways, orphaning the bridge, and breaking its source and test paths; against the committed registry it asserts the live bridge is due before the seal with exactly one declaration occurrence, that every declared phase is decidable, that NO stage carries a typed measurement -- the two stale copies this stage removed -- and that the SEAL GATE blocks while a bridge is declared independently of the plot states, which was a hole in the gate itself", alwaysRun: true },
  // P2-E: the road class, and the falsification of every rule that stops it being a way to move a number.
  "tests/unit/city/capability-roads-validator.test.ts": { covers: ["runtime"], obligation: "the road class is the cheapest way in this programme to make a metric move without repairing anything -- attribute an edge to the road class and a kernel stops depending on a building with no code change -- so a declaration must pass a NECESSARY condition the machine checks and a SUFFICIENT one it cannot; the necessary condition is LEAFNESS (imported across a capability boundary by two or more capabilities and importing NO other capability), which is ledger CC-030 refutation made executable, and the cases break it by declaring the shared sink that reaches nine capabilities; the sufficient condition is that the file carry NO POLICY OF ITS OWN, which cannot be automated without pretending to solve a semantic judgement, so the machine enforces the EVIDENCE -- five substantive proofs per road and a REFUTATION RECORD for the candidates that pass the leaf test and are still refused, with a case proving a refutation is rejected when the file is not a leaf or has a single consumer, because a record that records nothing is not evidence; leafness is NECESSARY AND NOT SUFFICIENT and that is the finding rather than a caveat: the committed artifact refuses src/shared/execution.ts, a leaf imported by four capabilities that exports reviewResponse and defaultReviewPolicy which DECIDE an outcome, and src/shared/permission.ts, a leaf that exports the security decision procedure; the remaining cases refuse a road owned by a KERNEL (already foundation, so declaring it a road moves nothing -- a gap the first version of the validator had and the falsification case found), a road owned by no capability (not trapped in a building), one whose declared owner disagrees with the ownership map, one that is also a composition-root file, one missing each of the five proofs in turn, one citing a ledger entry that does not exist, a classification too thin to be a rule, and a file declared a road and refuted at once; against the real tree it asserts leafless roads with at least two consumers each, that the reasoned-about members are still declared BY NAME while the set is free to grow, nine real refutations that every one of them PASS the leaf test, zero edges leaving a road, and that every road carries an exit condition naming the extraction that removes the declaration -- with the ratchet flooring road_files and edges_to_roads so a declaration cannot be withdrawn to push its edges back into a building column", alwaysRun: true },

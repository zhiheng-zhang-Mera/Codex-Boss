...
function checklist(root = ROOT, options = {}, cache = new Map()) {
  ...
  add("EVIDENCE", "E2", "all live renovation debt is CLOSED or ACCEPTED_PERMANENT",
    debtReady ? verdict(PASS, `${debtIds.length} debt id(s), all settled: ${perEntry.map((line) => line.split(/\s+/)[1]).join(", ")}`) : verdict(OPEN, unsettled.length > 0 ? `${unsettled.length} debt entr(ies) are still ${unsettled.map((line) => line.split(/\s+/)[1]).join(", ")}` : "the debt register declares no debt ids"));

...
export class ExecutionSupervisor {
  ...
  private async run(request: RuntimeRequest, candidates: RuntimeAdapter[]): Promise<RuntimeResult> {
    ...
    const compatible = candidates.filter((runtime) => runtime.capabilities.roles.includes(request.role));
    ...
    for (let index = 0; index < compatible.length; index++) {
      ...
      if (result.status === "SUCCESS" && review.status === "PASS") {
        this.ledger.update(request.taskId, "step completed", (value) => {
          ...
          value.completedSteps = [...new Set([...value.completedSteps, request.jobId])]; value.currentStep = null; value.nextAction = "NEXT_STEP";
      ...
      if (index + 1 < compatible.length) continue;

...
- **tests/unit/research-partial-failure.test.ts（新，真实 supervisor + ledger）**：
  - executor 早期阶段成功（SCOPING/PROJECT_INSPECTION/LITERATURE_REVIEW/QUESTION_FORMULATION/PROTOCOL_DRAFT/
    PROTOCOL_FROZEN，各 appendDecision + evidenceRefs）后于 EXPERIMENT_GENERATION fail-closed 抛错 ⇒ 终态
    FAILED（绝不 READY）；**早期全部决策与证据保留**（artifacts/<STAGE>.json），失败记录为
    failed:EXPERIMENT_GENERATION + reason，跨 reopen 可读不丢失（不因一段失败把项目抹掉）。
...
- **tests/unit/degraded-controller-battery.test.ts（新，真实 DegradedController + TaskLedger +
  BudgetManager，+5）**：双 eligible⇒FULL（池+maxWorkers+durable 持久化）/ 单 eligible⇒REDUCED(16000) /
  全 LOW⇒LIGHTWEIGHT(8000) / 预算耗尽⇒nativeAvailable?DETERMINISTIC:PAUSED（fail-closed 绝不 FULL）/
  applyTaskPolicy(L2) 后 evaluate 幂等（同输入同状态）+ policy.complexity 记录 + LOW 仍 eligible
  （仅 EXHAUSTED 出局）。
...
- **tests/unit/engineering-goal-rollback.test.ts（新，+2）**：
  - **§38 isolation**：git 基线 + 真实 MainCommander.runEngineeringGoal —— implementer 先改坏工作树再报错
    （候选崩溃中途）⇒ driver ABORTED ⇒ 稳定树被**完全回滚**：alpha.cjs 复原、summary.changedFiles=[]、
    git status 干净（候选副作用零泄漏）。
  - **§17 driver control**：注入全 ops（首轮 audit HIGH 发现 → implement 后 audit 清）⇒
...
## Round 41（2026-09-09，P1-x HumanGuidanceGate durable battery，owner-result @ 5b30a32）

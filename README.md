...
1. 先在 Windows 上登录并打开微信或 QQ 桌面客户端及目标聊天窗口。
2. 在 Codex Boss“设置 → PC 远程指令”中启用对应渠道，并保留或修改 `/boss` 前缀。
3. 聊天窗口出现 `/boss 整理今天的研究记录` 这类可见文本后，指令会进入主控输入框上方的待确认队列。
4. 点击“载入”才会把正文放入当前对话输入框；此时 AI 与网页/API 选择按原规则锁定，仍需用户主动提交。

该功能只使用 Windows UI Automation 检查本机可见的 `WeChat` / `Weixin` / `WXWork` 或 `QQ` / `QQNT` / `TIM` 窗口，只输出命令前缀后的文本。不读取账号密码、不复制登录 token、不调用非官方网络协议、不自动发送回复或执行命令。客户端未运行、未暴露可访问文本或 UI 结构变化时会显示 `waiting` / `error`，不会伪造已连接。个人微信/QQ PC 登录态不等同于腾讯官方机器人平台身份；需要无界面、高可用服务时应改接经过授权的官方机器人或企业回调接口。

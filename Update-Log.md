...
### Phase 7（§32）+ §30 回路 — Implementation Loop & Multi-Layer Review（CP9）

- **§32 审查决策层**（`src/shared/review.ts`，纯）：三层（Implementation Worker / Internal Reviewer / Adversarial Reviewer）、§32.1 五个维度 + 主题四项（视觉一致性、表面覆盖、回退安全、主题隔离）、§32.2 全部十二个对抗探针（每个探针都点名“必须查看哪些产物”）、§32.3 HIGH/MEDIUM/LOW/INFO 分派。§2.3 规则写进代码：HIGH/MEDIUM 若不能指名需求/文件**且**不能给出可复查指针（闸门/账本行/命令/报告）就**被拒绝**——不可证伪的抱怨无法阻塞；只有 Owner 能 `ACCEPTED_RISK`；没人审过的维度是 `NOT_RUN`（“看都没看”不是放行）；`completionGate` 要同时打开三扇门（无阻塞发现、覆盖完整、无欠证据需求）才允许 COMPLETED，输入里没有 `MODEL_DONE`。
- **宿主能自己判的检查**（`src/shared/review-checks.ts`，纯）：UNVERIFIED_COMPLETION（无任何通过闸门 **或** 其自身要求的、高于廉价基线的闸门从未通过 **或** 该闸门在本机根本爬不了）、correctness（闸门失败）、PARTIAL_STATE（验证失败但改动仍在盘上）、SILENT_FALLBACK（声称恢复/持久化却无重启或故障注入闸门）、scope、test_coverage、SECRET_EXPOSURE、architecture（手改生成产物）、maintainability、以及三个主题探针。`hostReviewRecords` 只报告真的看过的产物，所以 RACE / HIDDEN_FAILURE / SECURITY / FALSE_POSITIVE_TEST 默认仍是未审——除非另一层审查层交出带证据的记录。
- **两处由实跑校准的规则**：①**发现只有在修复权限内才允许阻塞**——“改了行为却没改测试”在授予作用域含测试路径时记 MEDIUM，域内没有测试路径时记 LOW（否则 worker 永远修不掉、回路空转）；②**仓库级闸门（syntax/typecheck）不能证明某个具体需求**，因此新增“该需求要求的、但从未通过或本机爬不了的高阶闸门”检查。
- **宿主审查引擎**（`electron/engineering/review-engine.ts`）：汇集真实产物——逐需求账本行与§31.2 闸门要求/不可用闸门、**真实写入字节**经既有 `scanSecrets` 扫描、生成目录分类（`GENERATED_DIRECTORIES`）、§20 主题校验状态、新文件判定；并按**当前状态**判定（每个闸门取最新一行，修好的失败不再阻塞，历史仍留在账本里），最后组装 §32 报告。同时修正 `recordEvidence`：重复观测不入第二行，但**刷新时间戳与度量**——否则“已修好的失败”会因为去重而被判为比它替换掉的通过更新。
- **§30 实现回路**（`electron/engineering/implementation-loop.ts`）：按计划书顺序驱动各级——受限作用域 → worker 提议 → 整体应用或整体拒绝 → 声明核验 → 逐需求爬梯 → §32 审查 → 阻塞发现回传 worker → 重验，默认上限 3 次（硬上限 5）；逐次记录 applied/被拒原因/改动与新文件/声明确认与驳回/闸门结果/阶梯结论/**阻塞发现的原文**/覆盖缺口/标签。标签来自 `completionGate`；**没有 worker 就是 `NOTHING_TO_DO` 并写明原因**，绝不是成功。
- **验收**：`pnpm run acceptance:review`（亦为 CI 门禁与本地链一步）在真实 git fixture + 真实工具链上跑 C-01..C-11 全 PASS（62 项观测）：真实改动被 git 确认、闸门实跑、因维度未审而只能是 INCOMPLETE；凭证形状被真实扫描器抓住→修复轮次→泄漏消失且阻塞解除；声称完成但要求重启闸门而本机无法爬 → HIGH UNVERIFIED_COMPLETION；越界写入不落盘且成为 scope 发现；typecheck 真实失败 → correctness + PARTIAL_STATE 且无完成；达到迭代上限即 REPAIR_REQUIRED；无提议/无 worker → NOTHING_TO_DO；worker 自写“DONE”永不产生 COMPLETED；对未触碰文件的声明被 git 驳回并上报；**第二审查层交出带证据记录后才可能 COMPLETED**；账本持久且从未由“声称”产生完成。单测 `tests/unit/review-layer.test.ts` 19 例。
- 诚实边界（写入 `docs/checkpoint-9-review-and-loop.md`）：这里的审查者是宿主自身（模型审查者只能经 `reviewerRecords`/`reviewerFindings` 追加记录与发现，且无法为自己没看过的维度放行）；**回路的在应用内消费属下一 checkpoint**（WorkBook/commander 仍走旧的 `verifyAndRepair` 接缝，未改动、无回归）；主题探针读的是注册表记录而非 CSS 语义；修复仍是“同一 worker 带着发现再试”（§33 失败分类与恢复阶梯属 CP10）；迭代默认上限 3。

### Phase 8（§33）— Self-Healing / Recovery（CP10）

- **§33.1 统一失败分类**（`src/shared/recovery.ts`，纯）：`classifyFailure` 由**真实证据**判出 13 个类别之一——失败的闸门、工具自身输出、provider 运行时码、宿主亲自核对的工作区事实、主题校验计数、视觉结论、或明确的策略拒绝；并如实上报**决定该分类的信号**。判序按权威而非便利：策略/Guardian 拒绝最先判定，绝不会被输出里的某个词改判为 TRANSIENT；宿主核实的事实优先于措辞——错误里点名的模块若在盘上存在，`Cannot find module` 就判为 BUILD 而不是假的依赖缺口。没有类别可套时（例如 benchmark 未达标，§33.1 无 PERFORMANCE 类）如实记 UNKNOWN 并写明理由，不硬塞。
- **§33.2 恢复阶梯**：`planRecovery` 给出 native retry → local recovery → alternate internal path → alternate provider → degraded mode → HNS fallback → Hard Blocker 的**逐步**计划，每一步都带预算或“为何不适用”的理由——因此“直接跳到 HNS”是可见记录而不是隐含行为；最后一步永远是 HARD_BLOCKER。TERMINAL 无任何可用步骤且禁止 HNS；AUTH 与 WORKSPACE 把决定交还 Owner（`SIGN_IN` / `AUTHORIZATION`），因为本地修复无法替代登录或授权；THEME 额外带 §33.2 的主题阶梯（停用 → 回退内置 → 记录诊断）。`advanceRecovery` 强制顺序：只有更便宜步骤的预算用尽，后续步骤才会被提出；阶梯耗尽时报 Hard Blocker 而不是空转。
- **§33.3 HNS 定位**：`planHnsFallback` 可能因三种理由拒绝（该失败类别不可外包、未点名缺失能力、连续调用已达上限），但**无论允许或拒绝都返回 CapabilityGap** 与 §34 backlog 种子；连续上限默认 2，`recordHnsUsage` 只接受 §33.3 的四种角色，且对“没有 gap 的调用”直接抛错。
- **宿主侧**（`electron/engineering/recovery-engine.ts`）：从 §31.3 账本行取回**失败闸门自己的输出**；亲自核对工作区（输出点名的模块是否真在 `node_modules`、是否 git 仓库、逃逸/受保护路径、作用域拒绝、磁盘满）；策略拒绝取自**真实** §7.3 mutation guard（`assertMutationAllowed`）而非假设；把每一次 HNS 使用（允许或拒绝）连同 gap、§34 种子与角色追加写入 `<workspace>/artifacts/acceptance/capability-gaps.json`。
- **§30 修复阶段接线**：回路的修复步骤现在会请求分类并逐次记录（failure_class / severity / reason / 应做的恢复步骤 / hard_blocker / requires_owner），因此 §30 的 Repair 是被 §33 分类过的修复；未接 `recover` 回调则不记录任何分类（如实显示“没有分类”而不是编造）。
- **实跑暴露并修好的一处保真度问题**：`verifyClaims` 会把“把文件改回已提交内容”的改动判为未发生（此时 `git status` 干净），宿主于是驳回了一次真实发生的修改。现在该检查额外接受**变更单元自身记录的 before/after 哈希**（`changed_by_unit`）——这是更严格的推理而非放宽：单元从未触碰的路径（幻影声明）依旧被驳回。
- **验收**：`pnpm run acceptance:self-healing`（亦为 CI 门禁与本地链一步）用宿主真实产生的失败跑 RC-01..RC-10 全 PASS（64 项观测）：真实 `tsc --noEmit` 失败 → BUILD（信号含真实 `TS2322` 与闸门，应做步骤为 LOCAL_RECOVERY，不提供 provider 步骤）；真实缺失模块 → DEPENDENCY（模块名从输出抽取且宿主自查 `node_modules` 确认缺失）；真实 `node --test` 失败 → TEST（拒绝重试）；真实 mutation guard 拒绝本仓库 → TERMINAL/CRITICAL（无可用步骤、禁止 HNS、要求 Owner AUTHORIZATION）；真实主题校验报告 → THEME + 主题阶梯；阶梯不可跳过（一次失败后仍停在 LOCAL_RECOVERY，预算用尽才轮到 HNS，随后硬阻塞）；允许的 HNS 回退写入持久且带版本的 CapabilityGap（含 §34 阶段）；第三次连续 HNS 调用被按“拐杖”规则拒绝而 gap 仍然记录，且任何使用都不会没有 gap；实现回路逐次记录分类与应做步骤并在下一次修复成功，同时仍拒绝 COMPLETED；无法识别时保持 UNKNOWN 低置信度、阶梯耗尽报硬阻塞、计划从不自称成功，且 gap 跨引擎重载仍在。单测 `tests/unit/recovery-model.test.ts` 22 例。
- 诚实边界（写入 `docs/checkpoint-10-self-healing.md`）：分类是词面 + 宿主核实事实（不做语义建模，判不出即 UNKNOWN 并写明理由）；恢复步骤被**表达并排序**但并非全部被执行——本 checkpoint 决定“该做哪一步”并证明顺序与预算，alternate provider / degraded mode 的执行体已存在于别处（provider 恢复阶梯 R0–R8、`intervention.ts`），接线属在应用内的整合 checkpoint；HNS 只被**表达**（角色、强制 gap、拐杖上限、持久 backlog 均已实现并测试），真实外部执行体仍是既有 HNS 集成；§34 链条只被种下首级（improvement task 阶段 + frequency/severity 输入），完整 CapabilityGap → Improvement Task → Implementation → Regression Test → Knowledge Update → Capability Registry 属 CP11；provider 侧码复用既有 `provider-outcome` 词表。

### Phase 9（§34）— Capability Gap → Self Improvement（CP11）

- **§34 记录与聚合**（`src/shared/capability-gap.ts`，纯）：`aggregateGaps` 按**能力**（而非撞上它的任务）取键，因此“两个任务卡在同一个缺失能力”就是同一个 gap 出现两次——正是计划要求的 `frequency`；合并出现次数、最严重度、失败类别、任务、workaround 与首末时间；**没有点名缺失能力的记录不是任何知识**，直接丢弃。`qualifiesForImprovement` 实现“达到条件后”：重复达到阈值（默认 2 次）或单次已足够严重（HIGH+）即立项，并把两个阈值的实际取值写进理由，使决定可审计。
- **§34 链条**：六个阶段是有序序列（`chainProblems`/`nextChainStage`）——某阶段早于其前驱、或跳过了中间阶段，即便其他一切都绿也算问题。`planImprovement` 产出的改进任务带：gap 原文、实现回路据以评分的 §28 式交付物需求、作用域词、**将来判定收口的探针**、以及记录 gap 时观测到的能力判决。
- **§34 收口三扇门**（`closureFor`）：①回归测试通过**且引用 §31.3 证据行**；②知识更新为 ACCEPT 或 SUPERSEDE；③能力探针**确实移动**（MISSING/PARTIAL/RESERVED → 更好）。任一不满足即 `NOT_CLOSED` 并把链条退回 IMPLEMENTATION。**测试变绿但能力依旧缺失不算收口。**
- **§34 知识↔注册表**：`knowledgeCandidateFor` 把收口写成宿主可验证的知识（producer=VERIFICATION、authority=VERIFIED_HOST、引用探针证据）；未收口时刻意标为 UNVERIFIED 且更低置信度，使门禁无法把“未闭合的缺口”当成事实。`registryEntryFor` 记录判决前后、frequency、证据与 GAINED/PARTIAL/OPEN。
- **宿主侧**（`electron/engineering/improvement-loop.ts`）：读取 CP10 写下的 §33.3 backlog；用真实 §6.3 机制（`buildWorldModelWithGraph` + `probeCapability`）探测活工作区——因此“能力已获得”要求既有实现模块**又**有导入者，没人引用的模块只能是 PARTIAL；实现阶段跑**真实 §30 回路**（受限 worker → 应用 → 验证 → 审查）；回归阶段是一次全新的 §31 爬梯并引用其 PASS 行；知识更新经真实 `KnowledgeBase`/§5.3 门禁提交；写持久 `capability-registry.json`（同一能力的新条目替换旧条目）。
- **验收**：`pnpm run acceptance:capability-gap`（亦为 CI 门禁与本地链一步）在真实 git fixture（确实缺少该能力）上跑 CG-01..CG-10 全 PASS（64 项观测）：gap 来自真实 §33.3 运行（两次 HNS 回退，第二次被拐杖上限拒绝）并读回为同一能力的两次出现；阈值判定（重复→立项、单次 LOW→不立项、单次 CRITICAL→立项）；动手前探针为 MISSING 且任务记录了它；worker 什么都不提议 → 仍 MISSING → NOT_CLOSED / 退回实现 / 注册表 OPEN；**真实实现同时写入模块并接线** → 探针 MISSING→EXISTS、回归爬梯通过并带证据行、§5.3 门禁放行、链条以 CAPABILITY_GAINED 收口且六阶段有序；写好的模块编译不过 → 探针虽移动但被回归门挡住（用第二个能力，避免场景互相污染）；落库知识是真实 ACTIVE 的 `CAPABILITY_GAP` 对象（VERIFIED_HOST + VERIFIED + 证据齐全），而未收口那个保持 UNVERIFIED；带凭证形状的候选被 §5.3 门禁 REJECT 并因此挡住收口；注册表与 backlog 跨实例可读；只有达到条件的 gap 才会立项（一次性缺口只被记录、不进注册表）。单测 `tests/unit/capability-gap.test.ts` 18 例。
- 诚实边界（写入 `docs/checkpoint-11-capability-gap.md`）：worker 仍由外部注入（链条评判结果而非作者）；一次调用跑一轮，未收口即登记 OPEN 并退回实现，跨轮自动重开属在应用内整合；能力身份是词面（沿用 CP3 的严格多数词匹配），名字不出现在实现标识符里就判不出“已获得”，这是 fail-closed 方向；gap→知识每次收口尝试写一条事实，同一能力的历史事实由 §5.3 冲突机制裁决（门禁日志记录它走的是 ACCEPT 还是 SUPERSEDE）；未新增主题专用的 gap 路径——§34 说主题生成器的缺口走同一机制，它确实走同一条链（主题专用回归阶梯属 §17/§26 而非新链）。

### Phase 10 + Phase 11（§35 + §36）— Candidate State & Guardian Final Gate（CP12）

- **§35 生命周期是一台状态机而非约定**（`src/shared/candidate-gate.ts`，纯）：`TASK_LIFECYCLE`/`TASK_TRANSITIONS`/`advanceLifecycle` 实现 RUNNING → IMPLEMENTED → VERIFYING → REVIEWING → CANDIDATE → ACCEPTED；前进需要计划书对应的事件，`REPAIR` 把被拒的 Candidate **退回**（CANDIDATE → REVIEWING），其它事件一律拒绝并回显合法迁移集合——**`RUNNING → ACCEPTED` 根本无法表达**。`candidateRecordFor` 把 CANDIDATE 的含义写进持久记录：`awaiting_release_permission: true` 直到状态为 ACCEPTED，并带上需求、完成证据、已走步骤与 Guardian 判决。
- **§36 十一项检查在证据上求值**：`evaluateGuardian` 跑七项基础检查（goal compliance / requirement coverage / secret scan / scope validation / evidence completeness / destructive change check / Owner override compliance）加四项主题检查（theme isolation / fallback validation / no executable payload / built-in theme integrity），每项给出 `PASS`/`FAIL`/`NOT_RUN`、**看过哪些产物**与理由。两条规则承载教义：①**跑不了的检查记 NOT_RUN 并阻塞**——“没看”不等于“看过且没问题”；②只有所有必需检查都通过才是 `ACCEPTED`，否则是 `REPAIR` 并给出阻塞检查 id，正是 §36 要的修复输入。检查内容具体：覆盖率低于 100%、凭证形状、越界写入、欠证据/验证失败、§32 审查未覆盖、未经 Owner 批准的删除、作品未体现的 override、引用他主题的主题包、可执行载荷、无效回退计划、受损内置主题。
- **Owner override 合规是词面并如实声明**：被服务需求须携带该 override 的**多数显著词**（`termsOf` 至少一半），因为“作品是否体现 Owner 的原话”在本层无法语义判定。`theme_required` 让“触及主题车道但一个包都没读到”也必须走四项主题检查——否则主题改动可以通过“观测失败”逃掉检查。
- **宿主门禁**（`electron/engineering/candidate-guardian.ts`）从产物而非摘要取证：§31.3 账本（证据完整性；每个闸门取最新一行，修好的失败即清除；通过行作为完成证据）、§32 审查覆盖率、**真实密钥扫描器**扫过的写入文件、`git status` 的删除与 package.json 的脚本移除、编译契约的 overrides、以及真实主题包目录（原样交给 §20 校验器），并写持久 `<workspace>/artifacts/acceptance/candidate-record.json`。**此处自查出一个自身缺陷**：初版为校验器**合成** manifest（只给 id/name/version），导致每个主题都报 schema 错误——现在包被原样传递；Guardian 不得制造它要审判的证据。
- **验收**：`pnpm run acceptance:candidate`（亦为 CI 门禁与本地链一步）在真实产物上跑 GD-01..GD-10 全 PASS（52 项观测）：§35 跳步被拒且合法路径到达 ACCEPTED；空目标 + 空账本的候选无法放行且仍欠发布许可；证据齐备的候选被放行且七项检查都写明看过的产物、记录持久；真实 `AKIA…` 凭证形状使密钥扫描失败并进入修复输入；越界写入（与一次被拒的变更单元）使作用域校验失败；未覆盖的 §32 审查与无证据需求使证据完整性失败、覆盖到位即通过；真实 `git` 可见删除在 Owner 未批准时使破坏性变更检查失败、批准该路径后通过；未被体现的 override 失败、被需求携带其词后通过；真实主题包干净则四项主题检查全过、加入 `javascript:` 载荷后 §20 校验器报错且候选被拒、触及主题却什么都没读则记 NOT_RUN 而非通过；被拒的 Guardian 把生命周期退回 REVIEWING 并点名阻塞检查、发布许可仍欠。单测 `tests/unit/candidate-gate.test.ts` 17 例。
- 过程记录（如实）：修 fixture 字符串时误用 PowerShell `-replace`，把本 checkpoint 的验收文件重编码成 mojibake（`§`→乱码）并加了 BOM；已用按码点显式替换的 Node 脚本修好并去除 BOM。被破坏的正是仓库既有约定（不得用 PowerShell `Set-Content`/`-replace` 编辑 UTF-8 源码），而**发现它的正是本 checkpoint 自己的门禁**（GD-06 拿乱码字面量去比对实现里正确的 `§32`）。
- 诚实边界（写入 `docs/checkpoint-12-candidate-guardian.md`）：goal/override 合规是词面（语义读取器会更好；fail-closed 方向是“不相关需求无法满足任一检查”）；破坏性变更检查看的是路径删除与脚本移除，而非语义性破坏（把函数掏空不算删除）；Guardian 是门禁不是执行器——它给出判决与修复输入，把修复轮次跑回 CANDIDATE 属整合步骤；`reviewCovered` 由调用方（§32 报告）提供，因此门禁依赖审查真的跑过，没跑就失败；候选记录按次写入而非追加历史，候选尝试序列经步骤与账本可见。

### Phase 12 + Phase 13（§37 + §38）— Version Impact & Local Git Checkpoint（CP13）

- **§37 版本影响由宿主判定**（`src/shared/version-impact.ts`，纯）：每个因子一条规则，各自给出**它迫使的影响、理由与产生它的文件**——导出符号消失 / 公开面删除或重命名 / 随包发布的迁移 → MAJOR；新增导出或用户可见变化 → MINOR；普通实现改动 → PATCH；只动测试与文档 → NONE。结果是“最强的那条发现”，同时保留全部发现以便被质疑；`factors_examined`/`factors_not_observed` 记录到底看了哪些因子。**§37 的“不得让 Worker 随意决定”被强制**：worker 的声明与宿主自己的评估比对，冲突进入 `rejected_claims` 并附宿主理由（一致的声明不会被拒绝，因此这个字段是有意义的）。两个主题特例：只含主题包的改动封顶 NONE/PATCH；触及 Theme Engine 契约的改动置 `requires_reevaluation`，使主题的答案不能被继承。`suggestedVersion` 把影响映射为真实 semver。
- **§38 检查点契约**（`src/shared/git-checkpoint.ts`，纯）：`CheckpointRecord` 正是计划书列的那几项（HEAD、分支、diff 哈希与大小、task、candidate、evidence），外加改动文件与 §37 影响，使发布可以引用“为何允许”。`planRollback` 决定回滚会做什么：从记录的 HEAD 还原路径是安全操作、在分支未前移时已足够；分支前移则需要 hard reset，且只有在**Owner 明确批准**时才被计划；不同分支或空改动集会带理由拒绝。`guardGitHubWrite` 是 §38 的前置条件：该任务最新的检查点必须存在、在当前分支、且仍描述当前 diff，否则拒绝远端写入。`isUsableCheckpoint` 把不可读或版本不符的记录当作不存在，绝不当作有效。
- **宿主侧**（`electron/engineering/git-checkpoint.ts`）跑真实 git 并检视真实改动：`assess()` 为每条改动路径记录改动类型、是否属于被消费的面、是否声明 schema、是否用户可见、是否测试/文档或主题包，以及**改动前后的导出符号**（before 取自 `git show <head>:<path>`，after 取自磁盘）；`create()` 捕获真实 HEAD/分支/diff（有上限）与其哈希、改动文件，并按 id 持久落盘；`guard()` 用活的 diff 哈希比对；`rollback()` 按记录 HEAD 逐路径还原，并且——**实跑暴露的缺口**——**删除在该 HEAD 并不存在的路径**（“还原”无法撤销一次新增），同时回报结果 HEAD/分支/脏状态使结果可核验，且在 hard reset 前要求 Owner 批准。
- **验收**：`pnpm run acceptance:version-checkpoint`（亦为 CI 门禁与本地链一步）在真实仓库上跑 VC-01..VC-08 全 PASS（50 项观测）：只改文档为 NONE、新增导出为 MINOR 且点名符号；删除导出为 MAJOR 且 worker 的 `PATCH` 声明被拒并附理由、建议 bump 为 2.0.0；主题包为 NONE/PATCH、主题契约改动置 `requires_reevaluation`；检查点记录真实 HEAD（`git rev-parse`）、真实分支、非空且有哈希的 diff、task、candidate、evidence 与 §37 影响，且新实例能读回；无检查点的 push 被拒、有当前检查点被允许、编辑造成 diff 漂移后再次被拒；回滚把真实工作树还原到已提交内容且 HEAD 未动、状态干净；检查点之后已有提交时回滚被拒，直到 Owner 批准才真正 reset 回检查点且后续编辑消失；§30.3 的变更单元回滚与 §38 的检查点回滚在同一文件上一致（逐字节还原后工作树干净）。单测 `tests/unit/version-impact.test.ts` 15 例。
- 诚实边界（写入 `docs/checkpoint-13-version-checkpoint.md`）：影响规则是**结构性而非语义性**的（读改动类型、导出、路径与面分类；不触碰这些却又改变行为时只会落到 PATCH——fail-closed 方向是“可观察的破坏性改动必然到 MAJOR”）；`public_surface`/`schema` 是按路径分类（`src/shared`、`src/renderer`、`schema`/`types.ts` 命名），别的 monorepo 约定需要自己的分类（观测类型允许）；diff 捕获有上限（512 KiB）并按捕获文本哈希，因此超大 diff 只能由大小发现、不会完整保存在记录里；检查点本身**不推送任何东西**——§38 是 §39/§40 远端写入的前置条件，把 `guardGitHubWrite` 接到真实 push 路径属下一 checkpoint，在那之前由调用方遵守；hard reset 必须带 `owner_approved_discard: true`，回路自身没有任何路径可以丢弃提交。

### Phase 14 + Phase 15（§39 + §40）— GitHub App Execution & PR Automation（CP14）

- **§39.1 分支策略**：`branchNameFor` 产出 `boss/<task-id>/<slug>`（slug 有界且净化），`checkBranchPolicy` 拒绝其它一切——外来前缀、四段路径、不安全的路径段。**§39.2 提交策略**：提交信息写明摘要、细节、§37 的 `Version-Impact` 与四个 trailer（Task/Requirements/Candidate/Evidence）；`validateCommitMessage` 拒绝丢掉任一 trailer、未点名需求、或未引用证据的提交。**§40 PR 正文**：按计划书顺序输出八个 section（Goal / What changed / Requirements / Tests / Evidence / Known limitations / Risk / Rollback），涉及 Theme Engine 时再加四个（UI surfaces affected / visual evidence / fallback behavior / theme migration）；`validatePullRequestBody` 拒绝缺失或留空的 section。
- **一条发布序列**（`planRelease`）：CHECKPOINT → BRANCH → COMMIT → PUSH → PULL_REQUEST → READ_CI，第一个无法自证的前置条件即终止计划——没有检查点、分支名不合策略、提交丢 trailer、PR 正文有洞，都会在**写入任何东西之前**挡住发布。
- **执行器**（`electron/engineering/release-runner.ts`）真的执行：§38 门禁不许就不做任何事（并记录原因）；创建策略分支、暂存并以计划好的信息提交；推送到**配置的远端**；在接了 gateway 与 repository 时经**真实 `GitHubGateway`**（`createPullRequest` → `POST /repos/<owner>/<name>/pulls`）开 PR；写持久发布记录（分支、基线、检查点 id、提交 sha、PR 号/URL 与正文哈希、计划哈希、逐步详情、PUBLISHED/PARTIAL/REFUSED 判决）；没有远端就如实说“工作留在本地”，绝不虚构远端。
- **验收**：`pnpm run acceptance:publish`（亦为 CI 门禁与本地链一步）**PB-01..PB-10 全 PASS（62 项观测）且全程无网络**：远端是磁盘上的真实 bare 仓库，gateway 的 transport 是录制器。分支策略与净化、四 trailer（去掉 Requirements 即被拒）、八个 section（留空即被拒）与主题四段；无检查点即 REFUSED 并记录原因；真实推送后 `git ls-remote` 能看到 `refs/heads/boss/t-14/...`，且远端提交信息仍带四个 trailer；真实 gateway 先 `POST /app/installations/67890/access_tokens` 再 `POST /repos/owner/name/pulls`，`head` 是策略分支、`base` 正确、payload 里带 §40 正文，审计日志记下 `pull_request.create`（App JWT 用测试内内存生成的 RSA 密钥**真实签名**）；白名单外仓库被 `REPOSITORY_DENIED` 拒绝且**一个 HTTP 请求都没发**；guardian 策略拒绝 `repository.delete`，且 gateway **根本没有**破坏性方法（能力是“不存在”，不只是“被守”）；不完整的计划与漂移的工作树都被拒并点名阻塞步骤；发布记录持久且可被新实例读回。
- 诚实边界（写入 `docs/checkpoint-14-publishing.md`）：gateway 调用是**离线证明**的（跑的是真实 gateway 代码、认证路径与 payload，但 transport 是录制器；live 路径仍需要真实 App 安装，CI 不跑）；推送目标是**配置的远端**而非必然是 GitHub（`acceptance:publish` 故意把 push 指向本地 bare 仓库，使证明不需要任何凭据）；发布**尚未读取 CI 结果**（§41：READ_CI 步骤已被计划且前置条件被检查，但 parse → classify → repair → push 回路属下一 checkpoint）；`guardGitHubWrite` 守的是执行器的第一步而非每次 API 调用——gateway 自身的 guardian 策略仍逐次决定，因此即使计划允许、被拒操作也照样被拒；提交的 `Evidence` trailer 列的是**指针**而非内容，因此提交体积小，§31.3 账本仍是证据库。

### Phase 16（§41）— CI Repair Loop（CP15）

- **§41 解析**（`src/shared/ci-repair.ts`，纯）：`parseCiFailure` 按日志原样读取——失败的 step（`##[error]/##[group]`）、CI 实际跑过的命令、带文件/行/错误码的编译诊断（`file(line,col): error TS####` 与 `file:line:col: error CODE:` 两种形状）、失败测试名与断言信息、CI 自己的包装注解（`Process completed with exit code N`）与退出码。不认识的日志产出**空但诚实**的解析而不是编造原因；连作业都没起来的情况标记为 `infrastructure`；每次解析带 `signature` 哈希，使同一个失败可被识别。
- **分类与计划**：`classifyCiFailure` 把解析结果交给 §33 的失败模型（并从日志推断闸门：有失败测试→UNIT，有 TS 码→TYPECHECK），基础设施失败直接按日志判为 ENVIRONMENT；`planCiRepair` 把类别映射为 §41 必须重跑的本地闸门（BUILD→TYPECHECK、TEST→UNIT、WORKSPACE→SYNTAX+TYPECHECK…）、点名失败涉及的文件与测试，并用 §33.2 的步骤预算决定 REPAIR 还是 **HARD_BLOCKER**。TERMINAL 永不修复，直接硬阻塞并点名 Owner。`ciVerdict` 严格实现“CI: PASS?”：只有 success 才算通过，**读取失败不是通过**——瞎猜原因比停下更糟。
- **回路**（`electron/engineering/ci-repair-loop.ts`）用前面各 checkpoint 的真实机件跑计划书的循环：读 CI（§39 gateway 或注入读取器）→ 解析 → 分类 → 修复（调用方的受限 worker，经宿主应用）→ **本地验证**（§31 爬梯，只跑分类要求的闸门）→ 推送（§38/§39/§40 执行器）→ 再读 CI；到 PASS 或 Hard Blocker 为止，逐次记录 run id/结论/step/类别/签名/各闸门结果/修复文件/提交/判决，写持久记录，并把 CI 结果记为 §31.3 证据。**实跑纠正了一处设计**：推送需要**在推送时刻**读取发布状态——在回路自身修复之前取的检查点按定义已经过期（§38 正确地拒绝它），因此 `releaseInput` 改为回调，由调用方在写入前一刻取检查点。
...
### Phase 18（§53）+ §51 一次性计划 — Soak Test（CP17）

- **§53 规则**（`src/shared/soak.ts`，纯）：`SOAK_STAGES` 是计划书顺序的七段（fresh clone → bootstrap → task → repair → PR → CI → completion），`SOAK_METRICS` 是那六项计数器。`evaluateSoak` 拒绝被糊弄：一轮只有在**每一段都 ok 且至少带一个证据指针**时才算干净；soak 只有在干净轮数（含要求的主题轮）足够时才算通过；**六项指标必须全为零**；任何一段不得从未被演练；而**失败签名重复出现即提前终止**——把一条已知坏路重复二十遍不是 soak。`failureSignatureOf` 给每轮的失败一个稳定身份，使“重复”可被识别。`benchmarkPlan()` 把 §51 的十八条目录变成有序且标注主题的计划（B16–B18 为主题轮）。
- **宿主 runner**（`electron/engineering/soak-runner.ts`）：每轮真的做计划书的第一段——从 bare 远端**克隆**到全新目录，经真实 §31 爬梯把克隆“起起来”，用真实受限变更单元做一次任务，走一遍修复路径（先对 git 核验声明再回滚：新增文件靠删除撤销、改动靠还原），给出 §39 策略分支名，把自己那轮的 §31.3 账本当作 CI 结果，并在任何前序段失败时让 completion 段失败。每轮连同各段、证据与失败签名持久记录，并附 §51 计划。
- **验收暴露并修好的两处健壮性缺口**：runner 从不创建自己的临时目录（于是把 workspace 指错时给出**空的原因**），以及 spawn 错误被从原因里丢掉；另外 REPAIR 段的判定原本要求“restored > 0”，而撤销一次**新增**是删除——两者都算回滚。

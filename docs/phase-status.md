...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **thirteen** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc`, `settings-ipc`, `theme-ipc`, `task-lifecycle-ipc`, `research-owner-ipc` and `task-state-ipc`. `main.ts`: 1884 → 1797 → 1753 → 1754 → 1625 → 1601 → 1580 → **1528 lines**, and the inline handler count is down from 51 to **4**. The newest slice took the task state machine — launch, the Chat→Work upgrade decision, the running/paused/cancelled transitions, and accepting held evidence — and removed a duplication with it: the `executeDeterministic → executePlan → dispatchTask → continueIfReady` chain had been written out twice inline and is now one helper. **4 handlers remain**: `create-task`, `dispatch-task`, `research-start`, `research-compile-pdf`, which share helpers with the task-creation path (`taskTransports`, `workbookAttachments`, `titleForTask`) and are therefore the last block that has to move together |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **88 channels across the thirteen modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Each module states its dependency surface rather than re-declaring it: `ThemeSurface` derives from the real `ThemeService` with `Pick`, and the task-lifecycle and research-owner modules derive their record types from the functions that consume them. The task-state module's surface is wide by necessity — driving a task really does touch the store, commander, automation and recovery scheduler — and each method is typed to the **real** return (`unknown` for results this module only ever asks about by truthiness) rather than to a convenient boolean, a mistake typecheck caught three times across two rounds. Real defects found: `boss:update-role-route` took an **untyped** role; the theme module's tests pin that a duplicated id cannot escape the `custom-` namespace; and the research-record test recorded that `ResearchContractStore` uses a **directory per run**, one of the mixed layouts under `.boss/research/**` that Phase I lists. **4 handlers remain** (`create-task`, `dispatch-task`, `research-start`, `research-compile-pdf`) |

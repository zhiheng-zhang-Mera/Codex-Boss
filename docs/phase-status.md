...
| --- | --- | --- |
| F/G | Continue the extraction group by group: `bootstrap/runtime.ts` (roots + window lifecycle), `providers.ts` (provider pool, views, automation and the GitHub machine identity), `engineering.ts`, `research.ts`, `knowledge.ts` (knowledge base, world model, UI surfaces, themes), `automation.ts` (recovery scheduler, telemetry/experience recorders), and the session/runtime services | Started with the durable stores because they are the largest cluster of construction in the boot block and the safest to move: they need a data root and injected crypto, no window and no service. Each further group needs the acceptance chain re-run, and the book caps this phase at "受控拆分 / 不得大重写" |

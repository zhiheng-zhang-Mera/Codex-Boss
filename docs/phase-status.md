...
- **One git entry, and the migration is now enforced.** `electron/git/git-gateway.ts` is
  ...
  `electron/input/github-resolver.ts` and
  `electron/promotion-gate/github-promotion-adapter.ts` all route through it. The guard
  test walks every `.ts` file under `electron/` and fails a direct git spawn that is not
  in a declared debt list; a companion test fails when a listed file no longer spawns git.
...
| M | The three Root Trust Surface files still spawning git directly (`acceptance-session`, `autonomous-evolution-identity`, `autonomous-evolution-runner`) | Each move changes a file the trust epoch anchors, so it is an epoch-carrying change with re-certification rather than a refactor — the sequencing the plan itself calls for. Every other module in the application now asks the gateway, and the rule is enforced repo-wide |
...
| 3 files still spawning git directly / 3 process runners | `electron/**` | Phase M; seventeen modules use the gateway and the rule is enforced repo-wide. The three left are all Root Trust Surface, so each needs an epoch advance with re-certification |

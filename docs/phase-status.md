...
| H — error model | no core failure swallowed | **PARTIAL** | every fail-open guard closed (unreadable CODEOWNERS, `assertEngineeringWorkspace`, the recovery ledger, the research ledger's damaged runs, the event bus's dropped handler rejections, `replaceGoal`'s archive, the two lost durable records in `main.ts`); the audit's remaining must-fix sites are listed below. **Three more closed this round**, all of one class — a failure that produced the same answer as a benign state: `context-manager`'s restore (a corrupt context file read exactly like no context file), `final-acceptance-gate` (a report that is PRESENT but unparseable was filed under `artifacts_missing`, and a corrupt persisted record read as a gate that had never run) and `verification-engine`'s ledger loader (a corrupt ledger read as an empty one — and its comment claimed a distinction the code did not make). In each case the **decision was already fail-safe and is unchanged**; what changed is that the reason is now recorded and reachable, so "unverified" cannot be misread as "no evidence exists" while the evidence sits on disk |
...
| H | The audit's remaining must-fix swallowed failures: `main.ts` headless preflight (`updateRun`/`setTaskStatus`), `candidate-supervisor` journal write, `evaluation-store` golden, `recovery-engine` corrupt backlog, `candidate-guardian` git-status/package.json, `autonomous-evolution-surface/identity` unreadable files, `self-evolution-coordinator` evidence persist. Two more found while closing this round's three, both in `electron/input/repo-manifest.ts` and both the same class: `RepoManifestStore.restore()` reads a corrupt manifest as "never scanned" (`manifest = undefined`), and `statWorkspace` drops a file whose `stat` throws — and a directory whose `readdir` throws — from the manifest entirely, so "unchanged" can be wrong in the under-reporting direction. The `sha256: "unreadable"` marker at the hashing step is already honest and is NOT the defect the audit named | Each one changes a failure path that an acceptance gate currently exercises; they are triaged by severity and the highest were fixed first. Done so far: `context-manager` restore, `final-acceptance-gate` (unreadable vs missing reports, and the persisted record), `verification-engine` corrupt ledger, `root-authority/protected-surface-guard` |
...
| 28 must-fix swallowed failures, 4 closed | see the audit list in this round's commit message, and the Phase H row above for what each fix changed | each changes a failure path that acceptance currently exercises; triaged by severity, highest fixed first |

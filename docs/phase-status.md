...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **ten** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc`, `settings-ipc` and `theme-ipc`. `main.ts`: 1884 → 1797 → 1753 → 1754 → **1625 lines**, and the handler count is down from 51 to **22**. The theme slice is the largest and the one that moved real orchestration: the §19 UI-engineering escalation, the §15 base-package requirement, the §24 knowledge recording, the preview revision loop and its remembered intent all left the composition root, along with the pure preview mapper. What stayed is what needs an Electron object or an app path — the capture machinery (a `BrowserWindow` and `WebContents`), the UI-surface registry the `ThemeService` is itself built with, and where the capture directory lives — because a boot module must never import Electron. The remaining ~1625 lines still need the same treatment group by group |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **70 channels across the ten modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Each module states its dependency surface rather than re-declaring it: the theme module derives its `ThemeSurface` from the real `ThemeService` with `Pick`, so a renamed method breaks it at compile time. Two slices found real defects: `boss:update-role-route` took an **untyped** role, and the theme module's tests pinned that a duplicated theme's id is forced into the `custom-` namespace with no path separator surviving. The remaining handlers (task lifecycle, research start/compile, intervention resolution — 22 in total) are unchanged |

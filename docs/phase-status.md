...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **nine** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc` and `settings-ipc`. `main.ts`: 1884 → 1797 → 1753 → **1754 lines**. Reported honestly: the newest slice is **line-neutral in `main.ts`** (1753 → 1754), because those ten handlers were thin one-liners whose explicit service surface costs about what the handlers did. Its value is in Phase G's terms — the validation and the refusals moved into a tested module — not in the file size, and the earlier slices that moved real orchestration are where the reduction came from. The remaining ~1750 lines still need the same treatment group by group |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **57 channels across the nine modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Two extracted modules deliberately keep their environment/host reads at the composition root (the proxy question) or behind an opaque surface (the web-AI pane, which the module may ask to reload but cannot reach into), because a boot module must never import Electron. One real defect fell out of the newest slice: `boss:update-role-route` took its role argument **untyped**, so a renderer could name a role that does not exist; it now takes the domain's own union. The remaining handlers (task lifecycle, theme, research start/compile, intervention resolution) are unchanged |

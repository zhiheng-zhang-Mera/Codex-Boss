...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **eleven** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc`, `settings-ipc`, `theme-ipc` and `task-lifecycle-ipc`. `main.ts`: 1884 → 1797 → 1753 → 1754 → 1625 → **1601 lines**, and the inline handler count is down from 51 to **14**. The task-lifecycle slice moved the evidence-bundle recovery (build-if-missing), the "nothing to rehydrate" refusal, and the Codex review's status transitions, including the rule that a failed review is recorded as failed *and* observed against the runtime so it can never be left at RUNNING. `boss:launch-task` deliberately stayed: it opens provider panes through the limit guard the pane manager owns, so moving it means moving that coupling rather than pretending it is absent. The remaining ~1600 lines still need the same treatment group by group |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **78 channels across the eleven modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Each module states its dependency surface rather than re-declaring it: `ThemeSurface` is derived from the real `ThemeService` with `Pick`, and the task-lifecycle module derives its task/artifact/bundle types from the evidence engine's own signatures, so a renamed method or changed argument breaks it at compile time. Real defects found so far: `boss:update-role-route` took an **untyped** role, and the theme module's tests pin that a duplicated theme's id is forced into the `custom-` namespace with no path separator surviving. **14 handlers remain** (task creation and dispatch, research start/compile, intervention resolution, owner dashboard, project state) |

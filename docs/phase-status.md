...
| M — side-effect boundary | few explicit entries per side effect | **PARTIAL** | `electron/git/git-gateway.ts` is the single entry for git: `runGit`/`runGitSync` never throw and return `{stdout, stderr, code, ok, spawnError}`, with explicit timeout/buffer bands. `spawnError` was added this round because the sites being migrated distinguish "git said no" from "git never ran", and an empty stderr cannot tell those apart. **Ten modules moved onto it**: `engineering/{git-checkpoint, soak-runner, world-model, candidate-guardian, release-runner, native-tools}`, `host/{host-probes, doctor, sentinel-capture}` and `self-evolution/self-target-resolver` — and three of those (`host-probes`, `doctor`, `sentinel-capture`) had been running `execFileSync` with **no timeout at all**, so an unbounded wait in a diagnostic path is now a bounded one that reports `unknown`. The guard is now **repo-wide**: any new direct git spawn anywhere under `electron/**` fails `tests/unit/git-gateway.test.ts` unless it is in a declared debt list, and a second test fails when a listed file no longer spawns git, so the list cannot rot. **Ten files remain**: the three Root Trust Surface ones (`acceptance-session`, `autonomous-evolution-identity`, `autonomous-evolution-runner` — each move carries a trust-epoch advance), `input/github-resolver` (takes a configurable git binary path) and `promotion-gate/github-promotion-adapter` (builds a per-push credential environment the gateway does not pass through), plus five callback-shaped sites (`repro-snapshot`, `root-recovery/rollback-controller`, `self-evolution/self-evolution-host`, `self-evolution/self-evolution-coordinator`, `stable-candidate/workspace-manager`) |
...
- **One git entry, and the migration is now enforced.** `electron/git/git-gateway.ts` is
  ...
  native-tools}`, `electron/host/{host-probes, doctor, sentinel-capture}` and
  `electron/self-evolution/self-target-resolver` all route through it. The guard test
  walks every `.ts` file under `electron/` and fails a direct git spawn that is not in
  a declared debt list; a companion test fails when a listed file no longer spawns git.
...
| M | The ten files still spawning git directly (listed in the M row above): three are Root Trust Surface and need an epoch advance, two need gateway capabilities not yet modelled (configurable binary, credential environment), five are callback-shaped | Ten modules moved this round and the rule is enforced repo-wide, so what is left is a named, shrinking list rather than an unmeasured pile |
...
| 10 files still spawning git directly / 3 process runners | `electron/**` | Phase M; the gateway exists, ten modules use it and the rule is enforced repo-wide, but the remaining ten each need an epoch advance or a gateway capability not yet modelled |

...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **fifteen** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc`, `settings-ipc`, `theme-ipc`, `task-lifecycle-ipc`, `research-owner-ipc`, `task-state-ipc`, `research-run-ipc` and `task-creation-ipc` — and one **domain** module, `electron/tasks/task-inputs.ts`. `main.ts`: 1884 → … → 1474 → **1442 lines**, and the inline handler count is down from 51 to **1**. The input rules both creation paths share (`taskTransports`, `titleForTask`, `conversationScopedInputRefs`, `workbookAttachments`) moved to the domain module rather than into either IPC module, because both callers need them and the other direction would have `main.ts` importing from an IPC module. They take their store lookups as an argument, so they are now testable without a composition root — and the attachment store is a *property* rather than a lookup function, because "no store attached" must stay distinguishable from "no path for this ref". **1 handler remains**: `dispatch-task`, which needs the pane manager, the GitHub materializer and the WorkBook dispatch services at once |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **91 channels across the fifteen modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Each module states its dependency surface rather than re-declaring it: `ThemeSurface` derives from the real `ThemeService` with `Pick`, the task-lifecycle and research-owner modules derive their record types from the functions that consume them, and `create-task` derives its `createTask` argument from `MainCommander` itself. The task-state module's surface is wide by necessity — driving a task really does touch the store, commander, automation and recovery scheduler — and each method is typed to the **real** return (`unknown` for results this module only ever asks about by truthiness), a mistake typecheck caught three times across two rounds. Real defects found: `boss:update-role-route` took an **untyped** role; a duplicated theme id could have escaped the `custom-` namespace; `ResearchContractStore` uses a **directory per run**, one of the mixed layouts under `.boss/research/**` that Phase I lists; and `main.ts` re-exported `assertDispatchGroupSize` "for callers that already import from the bridge" when **no caller did** — dead surface, now removed. **1 handler remains** (`dispatch-task`) |

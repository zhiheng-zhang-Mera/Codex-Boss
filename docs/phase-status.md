...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` exists: `boot-module.ts` (the `BootModule<T>` contract, health reporting, reverse-order disposal) plus `workspace-ipc.ts` and `attachment-ipc.ts`, both registered by `main.ts` and covered by `tests/unit/bootstrap-ipc-modules.test.ts`; the remaining ~1800 lines still need the same treatment group by group |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | the eight extracted channels are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; a guard forbids `ipcMain.handle` for them in `main.ts`, forbids Electron imports in boot modules and forbids fs/git/process work there. The remaining handlers are unchanged |

...
| F — split `main.ts` | controlled decomposition | **PARTIAL** | `electron/bootstrap/` holds `boot-module.ts` (the `BootModule<T>` contract, health reporting at boot, reverse-order disposal) plus **twelve** extracted slices: `workspace-ipc`, `attachment-ipc`, `conversation-ipc`, `provider-ipc`, `status-ipc`, `engineering-surface-ipc`, `research-ipc`, `host-status-ipc`, `settings-ipc`, `theme-ipc`, `task-lifecycle-ipc` and `research-owner-ipc`. `main.ts`: 1884 → 1797 → 1753 → 1754 → 1625 → 1601 → **1580 lines**, and the inline handler count is down from 51 to **8**. The newest slice moved the research records (contract persistence and its sufficiency audit, review rounds) and the Owner read-model (dashboard projection, answering a guidance request, project state), with the data root, the optional subsystems and the workspace lookup all injected. **8 handlers remain**: `create-task`, `dispatch-task`, `resolve-mode-proposal`, `update-task`, `accept-evidence`, `launch-task`, `research-start`, `research-compile-pdf`. `launch-task` is blocked on the pane manager's limit guard; `create-task`/`dispatch-task` share three helpers (`taskTransports`, `workbookAttachments`, `titleForTask`) and are the last large logic block, so they move together or not at all |
| G — IPC boundary | handlers validate/call/translate only | **PARTIAL** | **84 channels across the twelve modules** are `validate → service → publish` and take a narrow service surface instead of reaching into the composition root; guards forbid `ipcMain.handle` for them in `main.ts`, forbid Electron imports in boot modules, forbid fs/git/process work there, and require the autonomous goal channel to stay the only one that reaches the goal runner. Each module states its dependency surface rather than re-declaring it: `ThemeSurface` derives from the real `ThemeService` with `Pick`, and the task-lifecycle and research-owner modules derive their record types from the functions that consume them. Real defects found: `boss:update-role-route` took an **untyped** role; the theme module's tests pin that a duplicated id cannot escape the `custom-` namespace; and the research-record test recorded that `ResearchContractStore` uses a **directory per run**, one of the mixed layouts under `.boss/research/**` that Phase I lists. **8 handlers remain** (task creation and dispatch, research start/compile, mode proposal, task status, evidence acceptance) |

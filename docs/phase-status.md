...
| M — side-effect boundary | few explicit entries per side effect | **PARTIAL** | `electron/git/git-gateway.ts` is the single entry for git: `runGit`/`runGitSync` never throw and return `{stdout, stderr, code, ok, spawnError}`, with explicit timeout/buffer bands. `spawnError` was added this round because the sites being migrated distinguish "git said no" from "git never ran", and an empty stderr cannot tell those apart. **Fifteen modules moved onto it**: `engineering/{git-checkpoint, soak-runner, world-model, candidate-guardian, release-runner, native-tools}`, `host/{host-probes, doctor, sentinel-capture}`, `self-evolution/{self-target-resolver, self-evolution-host, self-evolution-coordinator}`, `repro-snapshot`, `root-recovery/rollback-controller` and `stable-candidate/workspace-manager` — and three of those (`host-probes`, `doctor`, `sentinel-capture`) had been running `execFileSync` with **no timeout at all**, so an unbounded wait in a diagnostic path is now a bounded one that reports `unknown`. The guard is now **repo-wide**: any new direct git spawn anywhere under `electron/**` fails `tests/unit/git-gateway.test.ts` unless it is in a declared debt list, and a second test fails when a listed file no longer spawns git, so the list cannot rot. **Five files remain**: the three Root Trust Surface ones (`acceptance-session`, `autonomous-evolution-identity`, `autonomous-evolution-runner` — each move carries a trust-epoch advance), `input/github-resolver` (takes a configurable git binary path) and `promotion-gate/github-promotion-adapter` (builds a per-push credential environment the gateway does not pass through) |
...
- **One git entry, and the migration is now enforced.** `electron/git/git-gateway.ts` is
  ...
  `electron/root-recovery/rollback-controller.ts` and
  `electron/stable-candidate/workspace-manager.ts` all route through it. The guard test
...
| M | The five files still spawning git directly (listed in the M row above): three are Root Trust Surface and need an epoch advance, two need gateway capabilities not yet modelled (a configurable binary path; a per-push credential environment) | Fifteen modules have moved and the rule is enforced repo-wide, so what is left is a named, shrinking list — and the last two are gateway features to add, not call sites to rewrite |
...
| 5 files still spawning git directly / 3 process runners | `electron/**` | Phase M; fifteen modules use the gateway and the rule is enforced repo-wide. Three of the five need an epoch advance, two need a gateway capability (configurable binary path, credential environment) |

...
| --- | --- | --- |
| F/G | Continue the extraction group by group: `bootstrap/runtime.ts` (roots + window lifecycle — 31 `mainWindow` references and the smoke/restart entry point make it the widest slice, so it wants its own round), `providers.ts` (provider pool, views, automation and the GitHub machine identity), `engineering.ts`, `research.ts`, `automation.ts` (recovery scheduler, telemetry/experience recorders), and the session/runtime services | Extracted in the order of "cohesive, no window, testable without Electron": the durable stores first, then the self-model. Each further group needs the acceptance chain re-run, and the book caps this phase at "受控拆分 / 不得大重写" |

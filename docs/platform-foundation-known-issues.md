...
| **Hosted evidence for the FIXED candidate (`a890314…`, code identical across all runs)** | **Three executions with slow-tier data, zero timeouts**: run 35442171945 attempt 1 (push) = **543 823 ms**; run 35442182999 attempt 1 (pull_request) = **548 153 ms**; run 35442171945 attempt 2 (push rerun) = **442 269 ms**. A fourth execution (run 35442182999 attempt 2) produced no 100k data because `unit` aborted earlier in `test:postbuild` on the unrelated `PF-DEBT-017` flake — recorded here, not fixed here. The spread between runs of identical code is **1.24×**, and the slowest observation carries only 8.6% headroom, so the entry stays OPEN and the decision moves to the tiering proposal. |

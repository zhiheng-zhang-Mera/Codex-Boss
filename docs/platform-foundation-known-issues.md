...
| **Status** | `FIX_PENDING_HOSTED_OBSERVATIONS` — the cost was measured, an implementation optimisation that preserves every contract was made, and closure now depends on hosted-runner observations of the same SHA (see the fix record below). No timeout, budget, retry, skip or workload change was made. |
...
| **Closure criteria** | `PF-DEBT-019` is closed only by hosted evidence: **at least three executions of the same candidate SHA on GitHub-hosted runners, none below the budget's margin, and zero occurrences of the timeout signature**. Hosted measurements of the UNFIXED code for the same case span **426 719 ms and 517 641 ms on green runs and >600 000 ms on the two timed-out ones**, against a local 87 522 ms — a hosted/local factor of 4.9–5.9×. If the fixed case still lands at 450–590 s, this entry stays OPEN and the gate-tiering proposal below is the next Owner decision instead. |

...
| **Status** | `FIXED` — Owner decision **B′** implemented and proven on BOTH sides. Hosted required CI proves the durable-event CONTRACT at a bounded volume and does **not** execute the 100k case (runs 35475634951 push and 35475646255 pull_request: all four required contexts green, `0` executed lines matching the 100k case in either log). The 100k scale claim is proven on the controlled real host through the private control plane (run 35476927229, step 19, `[real-host-scale] events=100000 appendMs=29883 readbackMs=646 reopenMs=38 totalMs=30648 result=PASS`) at the exact public-main SHA `7c718aa…`. Decision record: `docs/pf-debt-019-gate-tiering-proposal.md` §6; closure record below. |
| **Closure record — root cause and the fix that stayed** | The append loop was spending **848 µs per event** on work the contract never required: 3 prepared-statement compilations, 2 SELECTs and 1 INSERT per event — 300 000 compilations and 200 000 reads for 100 000 writes. `electron/state-core/event-journal.ts` now prepares its hot-path statements **once per journal** (one `DatabaseHandle` lifetime, so they cannot cross handles) and writes the common path with a single `INSERT … ON CONFLICT(producer, idempotency_key) DO NOTHING RETURNING *` against the UNIQUE index the schema already declared. A replay still returns the **ORIGINAL** durable row, pinned adversarially in `tests/unit/state-core/state-core.test.ts`. Measured: local case 87 522 ms → median 63 915 ms (5 runs), statement compilations 300 000 → 0, SELECTs 200 000 → 0; cost attribution put per-append JavaScript at 4.9 µs and showed the remainder is durable WAL/file I/O (57 µs in-memory vs 219 µs on disk at 20k events). A third change — preparing `BEGIN IMMEDIATE`/`COMMIT` once per handle — was **reverted because it did not pay for itself**. |
| **Closure record — why 600 s stayed unsuitable on shared runners** | Raising the budget would have been the wrong fix and was rejected by the Owner. With identical code the 100k case took **326 718 / 442 269 / 543 823 / 548 153 ms** on four GitHub-hosted runners — a **1.68×** spread against a 600 s budget, i.e. the budget sat *inside* the machine's own variance, while the fastest run finished in 54.5% of it. The contract is deterministic; the cost is the shared host's storage stack, and waiting up to ~18 minutes per push for it is not the desired merge contract. The real host ran the same case in **30 648 ms** — an ~11–18× difference — which is the measurement that justified moving the evidence rather than the ceiling. |
| **Closure record — what each side now proves** | **Hosted required CI (`REAL_HOST_SCALE` never runs here):** `tests/unit/platform/durable-event-correctness.test.ts` runs the shared contract at 10 000 events — measured hosted at **14 319 ms** against the default tier's 60 s ceiling — covering durability across close/reopen, strict sequence monotonicity, no loss or repeat across a paged readback, per-aggregate ordering, the UNIQUE(producer, idempotency_key) contract returning the ORIGINAL row, stats/head and rollback atomicity, and it documents in the file that it is correctness evidence and **not** the scale claim. **Real host (private control plane):** the unchanged 100k case (`EVENTS = 100_000`, unchanged `}, 600_000);`) as its own evidence class. Both call `tests/helpers/durable-event-contract.ts`, so the two classes cannot drift into two definitions of "the journal is correct". The `platform-qualification` tier is untouched and still ran (step 18, 3 files / 25 tests) — the two are separate declarations, not one borrowed from the other. |
| **Closure record — identity of the evidence and the boundary** | Closing commits: public `7c718aa3fafcbecc54d29f78f1052e9859514579` (the tiering change, landed by fast-forward from validation branch `perf/pf019-real-host-scale-tier`, PR #7), private control plane `a9541edc976fae3eddf9189ee2515fbe7d602653`. Real-host run **35476927229**: 24/24 steps, 0 skipped, `qualifiedCommit` = `publicMainSha` = `7c718aa…`, runner `boss-real-soak`, class `self-hosted`, trust epoch 23, quiescence `CHANGED_BY_THIS_RUN`; the redacted summary recorded `realHostScaleEvents: 100000`, `realHostScaleResult: "PASS"`, `realHostScaleDurationMs: 30648`. One harness incident is recorded rather than hidden: the step's **first** attempt (run 35476497454) aborted before the case ran anything — PowerShell 5.1 with the runner's `$ErrorActionPreference = 'Stop'` turns a native command's stderr into a terminating error when piped, and `node:sqlite` prints an `ExperimentalWarning` there; the fix redirected inside `cmd`, no test or threshold was touched, and attempt 2 produced the evidence above. |
| **Last reviewed SHA** | `7c718aa3fafcbecc54d29f78f1052e9859514579` |
...
| `7c718aa3fafcbecc54d29f78f1052e9859514579` | 01 (PF-DEBT-019 B′ — real-host scale tier) | none — `PF-DEBT-019` gained its closure record in place rather than as a new entry | **`PF-DEBT-019`** — closed by Owner decision B′: the append path was made measurably cheaper without touching a guarantee, the 100k case was extracted unchanged into a distinct `REAL_HOST_SCALE` execution tier, hosted required CI kept the contract at 10 000 events, and the scale claim was proven on the controlled real host in **30 648 ms**. `PF-DEBT-017` and `PF-DEBT-018` are untouched |

...
| **Status** | `RESOLUTION_IMPLEMENTED_AWAITING_REAL_HOST_EVIDENCE` — the Owner selected **Option B′**: the 100k case has been extracted **unchanged** into a distinct `REAL_HOST_SCALE` execution tier (`pnpm run test:real-host-scale`, executed by the private real-host control plane), the hosted required CI keeps a bounded **correctness** case for the same contract, and no workflow in this public repository may run the scale tier. `FIXED` requires BOTH sides — the hosted correctness case green **and** the 100k case green on the verified real host. Decision record: `docs/pf-debt-019-gate-tiering-proposal.md` §6. No timeout was raised, no workload reduced, no assertion removed. |

...
describe("provider completion collection", () => {
  it("observes and captures all waiting providers concurrently", async () => {
    ...
    const store = { snapshot: () => ({ tasks: [{ id: "task", status: "running" }] }), setRunSession() {}, runsForTask: () => runs, updateRun() {}, captureArtifact(runId: string) { captured.push(runId); const run = runs.find((item) => item.id === runId)!; run.phase = "completed"; }, failDispatchCollection() {} };

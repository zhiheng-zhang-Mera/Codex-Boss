...
function resolvedCase(caseId: string, componentId: string, failureMode: string, at = AT): CaseTimeline {
  const opened = openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId, at, trigger: "a reading over its limit", affectedComponents: [componentId] });
...
describe("a case is its timeline, folded", () => {
  it("opens with an event and folds to a status", () => {
    const opened = openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-1", at: AT, trigger: "a task ledger write failed", affectedComponents: ["tasks"] });
  ...
  it("refuses a case with no id, no trigger or an unparseable instant", () => {
    expect(openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: " ", at: AT, trigger: "x" }).problems.join(" ")).toContain("a case needs an id");
    expect(openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-1", at: AT, trigger: "  " }).problems.join(" ")).toContain("a case needs a trigger");
    expect(openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-1", at: "not-a-date", trigger: "x" }).problems.join(" ")).toContain("does not parse");
  });
  ...
  it("records what was performed separately from what was proposed", () => {
    ...
    const performed = appendEvent(withProposal.timeline as CaseTimeline, { at: LATER, type: "TREATMENT_PERFORMED", detail: { proposalId: "treatment:h", treatment: "RETRY", performedBy: "OWNER", outcome: "the run succeeded", reversible: true } });
    ...
    expect(done.performedBy).toBe("OWNER");
    // A treatment is performed by a person or another system: this plane may record one, never be it.
    expect(appendEvent(withProposal.timeline as CaseTimeline, { at: LATER, type: "TREATMENT_PERFORMED", detail: { proposalId: "treatment:h", treatment: "RETRY", performedBy: "SELF_DIAGNOSIS", outcome: "x" } }).problems.join(" ")).toContain("SELF_DIAGNOSIS is never one of them");
...
describe("recurrence is a link, and priors are about the past", () => {
  it("links a new case to the closed cases it recurs from", () => {
    ...
    const opened = openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-3", at: AT, trigger: "x" });
...
describe("the store appends events and folds them back", () => {
  it("records a case across a restart, one row per event", () => {
    ...
    const opened = store.openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-1", trigger: "a reading over its limit", affectedComponents: ["providers"] });
    expect(opened.ok).toBe(true);
  ...
  it("refuses to open the same case twice, and to append to a case it does not hold", () => {
    ...
    expect(store.openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId: "case-1", trigger: "y" }).problems.join(" ")).toContain("never opened twice, and a recurrence is a link");
  ...
  it("exposes candidates and priors from what it holds", () => {
    ...
    for (const caseId of ["case-1", "case-2"]) {
      store.openCase({ provenance: PROVENANCE, incidentClass: "DEVELOPMENT_TEST", caseId, at: AT, trigger: "x", affectedComponents: ["providers"] });

...
describe("Phase F/G — extraction actually moved code out of main.ts", () => {
  ...
  it("the extracted channels are registered by boot modules, not by main.ts", () => {
    const channels = [
      ...
      "boss:theme-snapshot", "boss:theme-activate", "boss:theme-duplicate", "boss:theme-delete",
      "boss:theme-restore-default", "boss:theme-validate", "boss:theme-generate", "boss:theme-preview",
      "boss:theme-preview-revise", "boss:theme-preview-accept", "boss:theme-preview-cancel",
      "boss:theme-capture", "boss:theme-visual-check",
      "boss:prepare-task", "boss:send-task", "boss:release-review", "boss:capture-task",
      "boss:advance-council", "boss:build-evidence", "boss:rehydrate-evidence", "boss:run-codex-review",
      "boss:research-contract-record", "boss:research-contract-audit", "boss:research-review-round",
      "boss:owner-dashboard", "boss:resolve-intervention", "boss:project-state",
      "boss:launch-task", "boss:resolve-mode-proposal", "boss:update-task", "boss:accept-evidence",
      "boss:research-start", "boss:research-compile-pdf",
      "boss:create-task", "boss:dispatch-task"
    ];
    ...
    expect(channels.length).toBeGreaterThanOrEqual(92);
  });

  it("main.ts registers no channel by name — every channel belongs to a boot module", () => {
    // The milestone this phase existed for. The root still CALLS ipcMain.handle: it is
    // the registrar it passes to each module. What it must never do again is name a
    // channel itself, because that is how 51 handlers accumulated here in the first
    // place. The list above catches a regression for a known channel; this catches a
    // brand-new one.
    expect([...main.matchAll(/ipcMain\.handle\(\s*"/g)].map((match) => match[0])).toEqual([]);
  });

  it("main.ts registers the modules and reports their health", () => {
    for (const factory of ["createWorkspaceIpcModule(", "createAttachmentIpcModule(", "createConversationIpcModule(", "createProviderIpcModule(", "createStatusIpcModule(", "createEngineeringSurfaceIpcModule(", "createResearchIpcModule(", "createHostStatusIpcModule(", "createSettingsIpcModule(", "createThemeIpcModule(", "createTaskLifecycleIpcModule(", "createResearchOwnerIpcModule(", "createTaskStateIpcModule(", "createResearchRunIpcModule(", "createTaskCreationIpcModule(", "createDispatchIpcModule("]) {
  ...
  it("the boot modules each expose service + health + dispose", () => {
    for (const file of ["electron/bootstrap/workspace-ipc.ts", "electron/bootstrap/attachment-ipc.ts", "electron/bootstrap/conversation-ipc.ts", "electron/bootstrap/provider-ipc.ts", "electron/bootstrap/status-ipc.ts", "electron/bootstrap/engineering-surface-ipc.ts", "electron/bootstrap/research-ipc.ts", "electron/bootstrap/host-status-ipc.ts", "electron/bootstrap/settings-ipc.ts", "electron/bootstrap/theme-ipc.ts", "electron/bootstrap/task-lifecycle-ipc.ts", "electron/bootstrap/research-owner-ipc.ts", "electron/bootstrap/task-state-ipc.ts", "electron/bootstrap/research-run-ipc.ts", "electron/bootstrap/task-creation-ipc.ts", "electron/bootstrap/dispatch-ipc.ts", "electron/bootstrap/persistence.ts", "electron/bootstrap/knowledge.ts", "electron/bootstrap/automation.ts", "electron/bootstrap/runtime.ts", "electron/bootstrap/providers.ts", "electron/bootstrap/provider-pool.ts", "electron/bootstrap/research.ts", "electron/bootstrap/engineering.ts"]) {
      const text = fs.readFileSync(path.join(PROJECT, file), "utf8");
